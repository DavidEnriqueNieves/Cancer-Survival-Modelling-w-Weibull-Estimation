...
wpf_cdf <- function(p,t) {
lam = p[1]
omega =  p[2]
phi = p[3]
nu = p[4]
result <- 1 - exp(-nu * (t^lam / (phi^lam - t^lam))^omega)
return(result)
}



ks.test(cdf_vals,cdf  )
plot(cdf_vals)
ks.test(cdf_vals,cdf  )
ks.test(cdf, cdf_vals  )
cdf_vals
ks.test(cdf, cdf_vals  )
ks.test(cdf, cdf_vals  )
ks.test(cdf_vals,cdf  )
p <- c(lambda, omega, phi, nu)
length(t)

create_cdf <- function(t) {
  data_cdf <- c()
  for(x in t){
    cat("TESTING X VALUE " , x , " \n")
    x_count <- 0
    for(i in t){
      # cat("i value is " , i ,"  \n")
      if(i <= x) {
        x_count <- x_count + 1
        # cat("x_count is " , x_count, "\n")
      }
    }
    x_count <- x_count / length(t)
    cat("x_count is " )
    cat(x_count, "\n")
    data_cdf <- c(data_cdf, x_count)
  }
  return(data_cdf)
  
}
plot(t, cdf_pred_vals)
...
p <- c(lambda, omega, phi, nu)
sorted_t <- sort(t)
cdf_pred_vals <- wpf_cdf(p, sorted_t)
data_cdf <- create_cdf(sorted_t)
data_cdf
length(data_cdf)
length(t)
plot(sorted_t, data_cdf)
lines(sorted_t, cdf_pred_vals)
plot( cdf_pred_vals, data_cdf, main="Overall CDF vs Data")
cat("Overall KS test results:  \n")
ks.test(data_cdf,cdf_pred_vals, alternative=c("two.sided" ))
...
plot(sorted_t, data_cdf)
lines(sorted_t, cdf_pred_vals)
plot( cdf_pred_vals, data_cdf, main="Male CDF vs Data")
cat("Male KS test results:  \n")
ks.test(data_cdf,cdf_pred_vals, alternative=c("two.sided" ))
...
plot(sorted_t, data_cdf)
lines(sorted_t, cdf_pred_vals)
plot( cdf_pred_vals, data_cdf, main="Female CDF vs Data")
cat("Female KS test results:  \n")
ks.test(data_cdf,cdf_pred_vals, alternative=c("two.sided" ))
